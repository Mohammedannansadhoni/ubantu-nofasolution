...
router.get('/badge-layout', function(req,res){
    ...
    Event.findById(eventId, function (err, event) {
        ...
        res.render('event/badge-layout', {eventIdForPrint:eventIdForPrint, scripts:scripts, messages:messages, hasErrors:messages.length>0, fields:fields, 
            showBarcode:showBarcode, barcodeLeft:barcodeLeft, barcodeTop:barcodeTop, showQrCode:showQrCode, qrCodeLeft:qrCodeLeft, qrCodeTop:qrCodeTop, uploadImageFields:uploadImageFields, uploadLogoFields:uploadLogoFields});
...
router.get('/print-badge-layout/:id', function(req,res){
    ...
    Event.findById(eventId, function (err, event) {
        ...
        res.render('event/print-badge-layout', {layout:'print-layout', messages:messages, hasErrors:messages.length>0, fields:fields, 
            showBarcode:showBarcode, barcodeLeft:barcodeLeft, barcodeTop:barcodeTop, showQrCode:showQrCode, qrCodeLeft:qrCodeLeft, qrCodeTop:qrCodeTop});
                    ...
                    Sequence.findOneAndUpdate({name:'qrCode'}, {$inc:{value:1}}, {new:true}, function(err, qrCodeSeq){
                        ...
                        EventData.findOneAndUpdate(query, update, options, function(err, eventData){
                            ...
                            res.render('event/print-badge', {layout:'print-layout', messages: messages, hasErrors: messages.length > 0,  
                                fields:fields, showBarcode:showBarcode, barcodeLeft:barcodeLeft, barcodeTop:barcodeTop, barcode:eventData.barcode, showQrCode:showQrCode, qrCodeLeft:qrCodeLeft, qrCodeTop:qrCodeTop, qrCode:eventData.qrCode, uploadImageFields:uploadImageFields, uploadLogoFields:uploadLogoFields});
                        });
                    });
                });
            }
...
router.post('/manage-fonts', function(req, res) {
    ...
    form.parse(req, function (err, fields, files) {
        ...
        Font.findOne({fontName: fontName, isActive: true}, function(err, existingFont) {
            ...
            } else if(fontType === 'custom' && files.fontFile && files.fontFile.name) {
                // Handle custom font file upload
                var oldpath = files.fontFile.path;
                var fileName = Date.now() + '_' + files.fontFile.name.replace(' ', '_');
