    ...
    Font.find({isActive: true}).sort({created: -1}).exec(function(err, fonts) {
        ...
        var displayFonts = fonts.map(function(font) {
            return {
                _id: font._id,
                name: font.fontName,
                fontFamily: font.fontFamily || font.fontName,
...
router.post('/manage-fonts-simple', function (req, res) {
    ...
    if (!fontName) {
        ...
        return res.redirect('/event/manage-fonts');
            console.log('Font name is empty or invalid');

    }

    var newFont = new Font({
        name: fontName.trim(), // FIX: Added for unique index
        fontName: fontName.trim(),
        fontFamily: fontName.trim(),
        fontType: fontType || (fontApi ? 'google' : 'custom'),
        googleFontApi: fontApi || null,
        fontFile: req.file ? req.file.filename : null,
        isActive: true
    });
