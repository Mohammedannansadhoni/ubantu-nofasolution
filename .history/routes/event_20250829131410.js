...
function generateCompleteRegistrationHTML(designData, event, eventId) {
    ...
    // Ensure all field display settings are present
    fieldDefinitions.forEach(function(fieldDef) {
        if(typeof event[fieldDef.name + '_showInOnlineRegistration'] === 'undefined') {
            event[fieldDef.name + '_showInOnlineRegistration'] = false;
        }
    });
    // Filter fields that should be shown in online registration
    var visibleFields = [];
    fieldDefinitions.forEach(function(fieldDef) {
        var showInOnlineRegistration = event[fieldDef.name + '_showInOnlineRegistration'] = false;
