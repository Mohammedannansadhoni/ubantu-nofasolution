    ...
    Event.findById(eventId, function (err, event) {
        Event.findById(eventId).exec(function(err, event) {
            ...
            fieldDefinitions.forEach(function(fieldDef) {
                if(event[fieldDef.name + '_showInOnlineRegistration']) {
            ...
            res.render('event/badge-layout', {
                ...
                fields: fields,
                showBarcode: typeof showBarcode !== 'undefined' ? showBarcode : false,
                barcodeLeft: typeof barcodeLeft !== 'undefined' ? barcodeLeft : 10,
                barcodeTop: typeof barcodeTop !== 'undefined' ? barcodeTop : 10,
                showQrCode: typeof showQrCode !== 'undefined' ? showQrCode : false,
                qrCodeLeft: typeof qrCodeLeft !== 'undefined' ? qrCodeLeft : 10,
                qrCodeTop: typeof qrCodeTop !== 'undefined' ? qrCodeTop : 10,
                uploadImageFields: typeof uploadImageFields !== 'undefined' ? uploadImageFields : [],
                uploadLogoFields: typeof uploadLogoFields !== 'undefined' ? uploadLogoFields : [],
                customFonts: typeof customFonts !== 'undefined' ? customFonts : []
            });
        });
                ...
                var Font = require('../models/font');
                if (!eventId) {
                    return res.status(400).send('No event selected. Please select an event first.');
                }
                Event.findById(eventId, function (err, event) {
                    if (err || !event) {
                        return res.status(404).send('Event not found');
                    ...
                    fieldDefinitions.forEach(function(fieldDef) {
                        ...
                        if(fieldDef.name === 'barcode') {
                            showBarcode = true;
                            barcodeLeft = event['barcode_left'] || 10;
                            barcodeTop = event['barcode_top'] || 10;
                        }
                        if(fieldDef.name === 'qrCode') {
                            showQrCode = true;
                            qrCodeLeft = event['qrCode_left'] || 10;
                            qrCodeTop = event['qrCode_top'] || 10;
                        }
                        if(fieldDef.name === 'uploadImage') {
                            uploadImageFields.push({
                                fieldName: 'uploadImage',
                                fieldLeft: event['uploadImage_left'] || 10,
                                fieldTop: event['uploadImage_top'] || 10,
                                fieldWidth: event['uploadImage_width'] || 100,
                                fieldHeight: event['uploadImage_height'] || 100
                            });
                        }
                        if(fieldDef.name === 'uploadLogo') {
                            uploadLogoFields.push({
                                fieldName: 'uploadLogo',
                                fieldLeft: event['uploadLogo_left'] || 10,
                                fieldTop: event['uploadLogo_top'] || 10,
                                fieldWidth: event['uploadLogo_width'] || 100,
                                fieldHeight: event['uploadLogo_height'] || 100
                            });
                        }
                    });
                    Font.find({isActive: true}).sort({created: -1}).exec(function(err, customFonts) {
                        ...
                        res.render('event/badge-layout', {
                            ...
                            qrCodeTop: qrCodeTop,
                            uploadImageFields: uploadImageFields,
                            uploadLogoFields: uploadLogoFields,
