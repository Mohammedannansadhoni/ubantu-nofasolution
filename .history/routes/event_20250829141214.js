                            ...
                            if(result.statusFlag!='Attended'){
                                update = {badgePrintDate:currentDate, statusFlag:'Attended', barcode:barcodeSeq.value, qrCode:qrCodeSeq.value, username: req.user.email};
                                        ...
                                        if(item.indexOf('_showInOnlineRegistration')>-1 && event[item]==true ){
                                            ...
                                            var fieldLabel = item.substring(0, item.indexOf('_showInOnlineRegistration') ) + '_label';
                                            var fieldType = item.substring(0, item.indexOf('_showInOnlineRegistration') ) + '_fieldType';
                                            var fieldValue = '';
                                            var fieldMandatory = item.substring(0, item.indexOf('_showInOnlineRegistration') ) + '_isMandatory';
                                            var field={};
                                            field['fieldName']=fieldName;
                                            field['fieldLabel']=event[fieldLabel];
                                            field['fieldType']=event[fieldType];
...
function generateCompleteRegistrationHTML(designData, event, eventId) {
    ...
    // Display all fields that are set to showInOnlineRegistration (no default filtering)
    var visibleFields = fieldDefinitions.filter(function(fieldDef) {
        return event[fieldDef.name + '_showInOnlineRegistration'] === true;
    });
