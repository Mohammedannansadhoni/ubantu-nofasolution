...
function generateCompleteRegistrationHTML(savedDesignData, event, eventId) {
    // Extract form fields configuration from event
    var fieldDefinitions = [
        {name: 'uniqueId', label: event.uniqueId_label || 'Unique ID', type: 'text'},
        {name: 'title', label: event.title_label || 'Title', type: 'dropdown'},
        ...
        {name: 'country', label: event.country_label || 'Country', type: 'select'},
        {name: 'badgeCategory', label: event.badgeCategory_label || 'Badge Category', type: 'select'}
    ];
    
    // Generate form fields HTML based on event configuration
    var formFieldsHtml = '';
    fieldDefinitions.forEach(function(fieldDef) {
        ...
        if(showInOnlineRegistration) {
            ...
            switch(fieldDef.type) {
                case 'email':
                    inputHtml = '<input type="email" id="' + fieldDef.name + '" name="' + fieldDef.name + '" required>';
                    break;
                case 'tel':
                    inputHtml = '<input type="tel" id="' + fieldDef.name + '" name="' + fieldDef.name + '" ' + requiredAttr + '>';
                    break;
                case 'url':
                    inputHtml = '<input type="url" id="' + fieldDef.name + '" name="' + fieldDef.name + '" ' + requiredAttr + '>';
                    break;
                case 'dropdown':
                    if(fieldDef.name === 'title') {
                        inputHtml = '<select id="' + fieldDef.name + '" name="' + fieldDef.name + '" ' + requiredAttr + '>' +
                ...
                case 'select':
                    if(fieldDef.name === 'country') {
                        inputHtml = '<select id="' + fieldDef.name + '" name="' + fieldDef.name + '" ' + requiredAttr + '>' +
                            ...
                            '</select>';
                    } else if(fieldDef.name === 'badgeCategory') {
                        inputHtml = '<select id="' + fieldDef.name + '" name="' + fieldDef.name + '" ' + requiredAttr + '>' +
                    ...
                    break;
                default:
                    inputHtml = '<input type="text" id="' + fieldDef.name + '" name="' + fieldDef.name + '" ' + requiredAttr + '>';
            }
            
            formFieldsHtml += '<div class="form-field" data-field-name="' + fieldDef.name + '">' +
                '<label for="' + fieldDef.name + '">' + fieldDef.label + (isMandatory ? ' <span class="required">*</span>' : '') + '</label>' +
                inputHtml +
                '</div>';
        }
    });
    
    // Use saved design data if available
    var eventTitle = (savedDesignData && savedDesignData.eventTitle) || event.eventName || 'Sample Event Name';
    var eventHeader = (savedDesignData && savedDesignData.eventHeader) || 'Welcome to Our Event Registration';
    var eventFooter = (savedDesignData && savedDesignData.eventFooter) || 'Thank you for registering for our event!';
    var logoUrl = (savedDesignData && savedDesignData.logoUrl) || 'https://via.placeholder.com/150x60?text=Event+Logo';
    var backgroundImage = (savedDesignData && savedDesignData.backgroundImage) || '';
    
    // Create complete HTML with exact same structure as designer
    return '<!DOCTYPE html>' +
'<html lang="en">' +
'<head>' +
'  <meta charset="UTF-8">' +
...
'</html>';
    ...
    // Extract custom content from the saved event data
    var eventTitle = event.customEventTitle || event.eventName || 'Sample Event Name';
    var eventHeader = event.customEventHeader || 'Welcome to Our Event Registration';
    var eventFooter = event.customEventFooter || 'Thank you for registering!';
    var logoUrl = event.customLogoUrl || 'https://via.placeholder.com/110x60?text=Logo';
    var backgroundStyle = '';
    
    if(event.customBackground) {
        backgroundStyle = 'background-image: url(' + event.customBackground + ');';
    }
    
    // Create the same HTML structure as the designer template but with saved data and functional form
    return '<!DOCTYPE html>' +
'<html lang="en">' +
'<head>' +
'  <meta charset="UTF-8">' +
...
router.post('/online-registration/:id/save-design', function(req, res) {
    ...
    var path = require('path');
    var eventId = req.params.id;
    var pageType = req.body.pageType; // 'registration' or 'confirmation'
    
    console.log('Saving design for event:', eventId, 'Page type:', pageType);
    console.log('Request body keys:', Object.keys(req.body));
    
    // Find the event first to get its data
    Event.findById(eventId).exec(function(err, event) {
        if(err || !event) {
        ...
        if (pageType === 'confirmation') {
            // Save confirmation page specific data
            updateData.confirmationCanvasHtml = req.body.canvasHtml;
            updateData.confirmationTitle = req.body.confirmationTitle || null;
            updateData.confirmationMessage = req.body.confirmationMessage || null;
            updateData.confirmationFooter = req.body.confirmationFooter || null;
            updateData.confirmationLogoUrl = req.body.logoUrl || null;
            updateData.confirmationCustomBackground = req.body.customBackground || null;
            
            // Save confirmation page HTML to file
            var confirmationHtmlPath = path.join(eventFolder, 'confirmation.html');
            fs.writeFileSync(confirmationHtmlPath, req.body.canvasHtml);
            console.log('Saved confirmation HTML to:', confirmationHtmlPath);
            
        } else {
            // Save registration page data
            updateData.canvasHtml = req.body.completePageHtml || req.body.canvasHtml;
            updateData.fieldsOrder = req.body.fieldsOrder || null;
            updateData.customEventTitle = req.body.eventTitle || null;
            updateData.customEventHeader = req.body.eventHeader || null;
            updateData.customEventFooter = req.body.eventFooter || null;
            updateData.customLogoUrl = req.body.logoUrl || null;
            
            // Handle background image saving
            if (req.body.backgroundUploaded && req.body.backgroundImage) {
                try {
                    var bgImageMatch = req.body.backgroundImage.match(/url\(['"]?(data:image\/[^'"]*?)['"]?\)/);
                    if (bgImageMatch && bgImageMatch[1]) {
                        var bgDataUrl = bgImageMatch[1];
                        var matches = bgDataUrl.match(/data:image\/([a-zA-Z]*);base64,([^\"]*)/);
                        if (matches) {
                            var ext = matches[1];
                            var data = matches[2];
                            var bgFileName = `background-registration.${ext}`;
                            var bgPath = path.join(eventFolder, bgFileName);
                            
                            fs.writeFileSync(bgPath, data, 'base64');
                            console.log('✅ Saved background image:', bgPath);
                            
                            // Update the URL in the database to point to the saved file
                            updateData.customBackground = `/event-designs/${eventId}/${bgFileName}`;
                        }
                    }
                } catch (error) {
                    console.warn('Error saving background image:', error);
                }
            }
            
            // Handle logo saving
            if (req.body.logoUploaded && req.body.logoUrl && req.body.logoUrl.startsWith('data:')) {
                try {
                    var logoMatches = req.body.logoUrl.match(/data:image\/([a-zA-Z]*);base64,([^\"]*)/);
                    if (logoMatches) {
                        var ext = logoMatches[1];
                        var data = logoMatches[2];
                        var logoFileName = `logo.${ext}`;
                        var logoPath = path.join(eventFolder, logoFileName);
                        
                        fs.writeFileSync(logoPath, data, 'base64');
                        console.log('✅ Saved logo image:', logoPath);
                        
                        // Update the URL in the database to point to the saved file
                        updateData.customLogoUrl = `/event-designs/${eventId}/${logoFileName}`;
                    }
                } catch (error) {
                    console.warn('Error saving logo image:', error);
                }
            }
            
            // Create the saved design data object
            var savedDesignData = {
                eventTitle: req.body.eventTitle,
                eventHeader: req.body.eventHeader,
                eventFooter: req.body.eventFooter,
                logoUrl: updateData.customLogoUrl || req.body.logoUrl,
                backgroundImage: updateData.customBackground || req.body.backgroundImage
            };
            
            // Create complete registration HTML with form using saved design
            var completeRegistrationHtml = generateCompleteRegistrationHTML(savedDesignData, event, eventId);
            
            // Save registration page HTML to file
            ...
            console.log('✅ Saved complete registration HTML to:', registrationHtmlPath);
        }
        fs.writeFileSync(registrationHtmlPath, completeRegistrationHtml);
        console.log('✅ Saved complete registration HTML to:', registrationHtmlPath);
    }
    
    // Extract and save assets (backgrounds, logos) from the design
    try {
        if (canvasHtml) {
            // Simple regex extraction for base64 images in background-image style
            var bgImageMatches = canvasHtml.match(/background-image:\s*url\(['"]?(data:image\/[^'"]*?)['"]?\)/i);
            if (bgImageMatches && bgImageMatches[1]) {
                var bgDataUrl = bgImageMatches[1];
                var matches = bgDataUrl.match(/data:image\/([a-zA-Z]*);base64,([^\"]*)/);
                if (matches) {
                    var ext = matches[1];
                    var data = matches[2];
                    var bgFileName = `background-${pageType}.${ext}`;
                    var bgPath = path.join(eventFolder, bgFileName);
                    ...
                    // Update the URL in the database to point to the saved file
                    if (pageType === 'confirmation') {
                        updateData.confirmationCustomBackground = `/event/event-designs/${eventId}/${bgFileName}`;
                    } else {
                        updateData.customBackground = `/event/event-designs/${eventId}/${bgFileName}`;
                    }
                }
            }
            
            // Extract and save logo images from src attributes
            var logoMatches = canvasHtml.match(/src=['"]?(data:image\/[^'"]*?)['"]?/gi);
            if (logoMatches) {
                logoMatches.forEach(function(match, index) {
                    var srcMatch = match.match(/src=['"]?(data:image\/[^'"]*?)['"]?/i);
                    if (srcMatch && srcMatch[1]) {
                        var logoDataUrl = srcMatch[1];
                        var matches = logoDataUrl.match(/data:image\/([a-zA-Z]*);base64,([^\"]*)/);
                        if (matches) {
                            var ext = matches[1];
                            var data = matches[2];
                            var logoFileName = `logo-${pageType}-${index}.${ext}`;
                            var logoPath = path.join(eventFolder, logoFileName);
                            ...
                            if (pageType === 'confirmation') {
                                updateData.confirmationLogoUrl = `/event/event-designs/${eventId}/${logoFileName}`;
                            } else {
                                updateData.customLogoUrl = `/event/event-designs/${eventId}/${logoFileName}`;
                            }
                        }
                    }
                });
        ...
        console.warn('Could not extract assets from HTML:', error.message);
    }
    
    // Update the database
    Event.findByIdAndUpdate(eventId, updateData, {new: true}, function(err, event) {
        if(err) {
            console.error('Error saving page design:', err);
            return res.status(500).json({success: false, message: 'Error saving design'});
        }
        
        console.log(`${pageType} page design saved successfully`);
        res.json({
            success: true, 
            message: `${pageType.charAt(0).toUpperCase() + pageType.slice(1)} page design saved successfully`,
            pageType: pageType,
...
router.get('/public-registration/:id', function(req, res) {
    ...
    Event.findById(eventId).exec(function(err, event) {
        ...
        // Check for saved registration HTML file first
        var eventFolder = path.join(__dirname, '../public/event-designs/', eventId);
        var registrationHtmlPath = path.join(eventFolder, 'registration.html');
        
        if (fs.existsSync(registrationHtmlPath)) {
            try {
                var savedRegistrationHtml = fs.readFileSync(registrationHtmlPath, 'utf8');
                console.log('✅ Serving saved registration design for event:', eventId);
                
                // Send the saved HTML directly
                res.setHeader('Content-Type', 'text/html');
                return res.send(savedRegistrationHtml);
                
            } catch (error) {
                console.warn('❌ Error reading saved registration design:', error.message);
                // Fall through to template rendering
            }
        }
        
        console.log('📋 No saved design found, using template fallback');
        
        // Fallback: Use template system if no saved design exists
        var BadgeCategory = require('../models/badge-category');
        var Country = require('../models/country');
        BadgeCategory.find({event: eventId, isActive: true}).exec(function(err, badgeCategories) {
            ...
            Country.find({isActive: true}).exec(function(err, countries) {
                ...
                console.log(`Generated ${fields.length} fields for public registration`);
                
                // Check for saved design files for this event
                var fs = require('fs');
                var path = require('path');
                var eventFolder = path.join(__dirname, '../public/event-designs/', eventId);
                var registrationHtmlPath = path.join(eventFolder, 'registration.html');
                var savedCanvasHtml = null;
                
                // Check if there's saved canvas HTML in the database first
                if(event.canvasHtml) {
                    savedCanvasHtml = event.canvasHtml;
                    console.log('✅ Found saved canvas design in database for event:', eventId);
                }
                
                console.log('📋 Using template with', savedCanvasHtml ? 'custom design' : 'default design');
                
                // Prepare additional data for the default template
                var templateData = {
                    event: event,
                    fields: fields,
                    badgeCategories: badgeCategories,
                    countries: countries,
                    eventId: eventId,
                    eventHeader: event.customEventHeader || 'Welcome to Registration',
                    eventFooter: event.customEventFooter || `Thank you for registering for ${event.eventName}!`,
                    formNote: 'Please provide accurate information. You will receive a confirmation email at the provided email address.',
                    eventTitle: event.customEventTitle || event.eventName,
                    logoUrl: event.customLogoUrl || null,
                    savedCanvasHtml: savedCanvasHtml // Add the saved canvas HTML
                };
                
                // Check if there's saved canvas configuration in database (fallback)
                if(event.canvasHtml) {
                    templateData.savedCanvasHtml = event.canvasHtml;
                    console.log('📋 Using database canvas HTML as fallback');
                }
                
                if(event.customBackground) {
                    templateData.customBackground = event.customBackground;
                }
                
                // If there's a saved fields order, reorder the fields accordingly
                if(event.fieldsOrder && Array.isArray(event.fieldsOrder)) {
                    var orderedFields = [];
                    event.fieldsOrder.forEach(function(orderItem) {
                        var field = fields.find(f => f.fieldName === orderItem.fieldName);
                        if(field) {
                            orderedFields.push(field);
                        }
                    });
                    ...
                    fields.forEach(function(field) {
                        if(!orderedFields.find(f => f.fieldName === field.fieldName)) {
                            orderedFields.push(field);
                        }
                    });
                    
                    templateData.fields = orderedFields;
                }
                
                res.render('event/public-registration', templateData);
            });
        });
    });
});
        ...
        Sequence.findOneAndUpdate(
            ...
            eventData.save(function(err, savedData) {
                ...
                if (savedData.email && savedData.email.trim() !== '') {
                    console.log('📧 Attempting to send confirmation email to:', savedData.email);
                    ...
                    if (transporter) {
                        ...
                        if (fs.existsSync(templatePath)) {
                            try {
                                emailTemplate = JSON.parse(fs.readFileSync(templatePath, 'utf8'));
                                console.log('✅ Using custom email template for event:', eventId);
                            } catch (error) {
                                console.warn('Error reading custom email template, using default:', error.message);
                                emailTemplate = event.emailTemplate || {};
                            }
                        } else {
                            emailTemplate = event.emailTemplate || {};
                            console.log('ℹ️ No custom email template found, using default');
                        }
                    ...
                    console.warn('No email address provided for registration:', registrationId);
                }
                ...
                if (isAjaxRequest) {
                    ...
                    res.redirect(`/event/public-registration/${eventId}/confirmation?reg=${registrationId}`);
                }
            });
        });
    });
        ...
        if (fs.existsSync(confirmationHtmlPath)) {
            try {
                ...
                if(registrationId) {
                    ...
                    EventData.findOne({event: eventId, registrationId: registrationId}).exec(function(err, registration) {
                        if(err) {
