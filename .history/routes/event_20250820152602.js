...
router.post('/online-registration/:id/save-design', function(req, res) {
    ...
    try {
        if (canvasHtml) {
            var tempDiv = require('jsdom').jsdom(canvasHtml);
            
            // Extract background images
            var canvasElement = tempDiv.querySelector('#canvas');
            if (canvasElement && canvasElement.style.backgroundImage) {
                var bgImage = canvasElement.style.backgroundImage;
                if (bgImage.includes('data:image')) {
                    // Save base64 background image
                    var matches = bgImage.match(/data:image\/([a-zA-Z]*);base64,([^\"]*)/);
                    if (matches) {
                        var ext = matches[1];
                        var data = matches[2];
                        ...
                        if (pageType === 'confirmation') {
                            updateData.confirmationCustomBackground = `/event-designs/${eventId}/${bgFileName}`;
                        } else {
            ...
            var logoElements = tempDiv.querySelectorAll('img');
            logoElements.forEach(function(img, index) {
                if (img.src && img.src.includes('data:image')) {
                    var matches = img.src.match(/data:image\/([a-zA-Z]*);base64,([^\"]*)/);
                    if (matches) {
                        var ext = matches[1];
                        var data = matches[2];
                        ...
                        if (pageType === 'confirmation') {
                            updateData.confirmationLogoUrl = `/event-designs/${eventId}/${logoFileName}`;
                        } else {
