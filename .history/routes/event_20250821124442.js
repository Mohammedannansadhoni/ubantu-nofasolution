...
router.get('/email-configuration', function(req, res) {
    res.render('event/email_configuration', { title: 'Email Configuration' });
});
        ...
        BadgeCategory.find({event: eventId, isActive: true}).exec(function(err, badgeCategories) {
            ...
            Country.find({isActive: true}).exec(function(err, countries) {
                ...
                fieldDefinitions.forEach(function(fieldDef) {
                    ...
                    // Force email field to be named 'email' for backend compatibility
                ...
                // Load design.json for this event
                var fs = require('fs');
                var path = require('path');
        ...
        Sequence.findOneAndUpdate(
            ...
            eventData.save(function(err, savedData) {
                ...
                if (!savedData.email || savedData.email.trim() === '') {
                    ...
                    if (isAjaxRequest) {
                        return res.status(400).json({success: false, message: 'Email address is required for registration.'});
                    } else {
                        return res.status(400).send('Email address is required for registration.');
                    }
                }
