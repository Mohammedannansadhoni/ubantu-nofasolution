...
router.post('/online-registration/:id/save-design', function(req, res) {
    ...
    if (pageType === 'confirmation') {
        ...
        updateData.confirmationCanvasHtml = canvasHtml;
        ...
        fs.writeFileSync(confirmationHtmlPath, canvasHtml);
        console.log('Saved confirmation HTML to:', confirmationHtmlPath);
        
    } else {
        // Save registration page data
        updateData.canvasHtml = canvasHtml;
        updateData.fieldsOrder = req.body.fieldsOrder || null;
        updateData.customBackground = req.body.customBackground || null;
        ...
        // Create complete registration HTML with form
        var completeRegistrationHtml = generateCompleteRegistrationHTML(canvasHtml, event, eventId);
    ...
    Event.findById(eventId).exec(function(err, event) {
        ...
        BadgeCategory.find({event: eventId, isActive: true}).exec(function(err, badgeCategories) {
            ...
            Country.find({isActive: true}).exec(function(err, countries) {
                ...
                // If custom design exists, serve it directly
                if (fs.existsSync(registrationHtmlPath)) {
                    try {
                        var savedDesignHtml = fs.readFileSync(registrationHtmlPath, 'utf8');
                        console.log('✅ Serving saved custom registration design for event:', eventId);
                        return res.send(savedDesignHtml);
                    } catch (error) {
                        console.warn('❌ Error reading saved design, falling back to default:', error.message);
                    }
                }
