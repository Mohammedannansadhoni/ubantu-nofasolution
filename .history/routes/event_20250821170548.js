...
router.post('/:id/public-register', function(req, res) {
                console.log('🟡 [DEBUG] About to save registration with eventObjId:', eventObjId, 'Type:', typeof eventObjId);
            ...
            try {
                Sequence.findOneAndUpdate(
                    ...
                    if (err) {
                        ...
                        if (isAjaxRequest) {
                            return res.status(500).json({success: false, message: 'Error processing registration', error: err.message, stack: err.stack});
                        } else {
                            return res.status(500).send('Error processing registration: ' + err.message);
                        }
                    }
                    ...
                    if (eventId) {
                        if (typeof eventId === 'string' && mongoose.Types.ObjectId.isValid(eventId)) {
                            eventObjId = mongoose.Types.ObjectId(eventId);
                        } else if (eventId instanceof mongoose.Types.ObjectId) {
                            eventObjId = eventId;
                        } else {
                            console.error('❌ Invalid eventId, cannot convert to ObjectId:', eventId);
                            if (isAjaxRequest) {
                                return res.status(400).json({success: false, message: 'Invalid event ID for registration.'});
                            } else {
                                return res.status(400).send('Invalid event ID for registration.');
                            }
                        }
                    } else {
                        console.error('❌ eventId missing in registration request');
                        if (isAjaxRequest) {
                            return res.status(400).json({success: false, message: 'Event ID missing in registration.'});
                        } else {
                            return res.status(400).send('Event ID missing in registration.');
                        }
                    }
                    console.log('🟡 [DEBUG] About to save registration with eventObjId:', eventObjId, 'Type:', typeof eventObjId, 'InstanceOf ObjectId:', eventObjId instanceof mongoose.Types.ObjectId);
                    var eventData = new EventData({
                        event: eventObjId,
                    ...
                    eventData.save(function(err, savedData) {
                        if (err) {
                            console.error('❌ [ERROR] Registration save failed:', err);
                            if (err.stack) console.error('❌ [ERROR] Stack:', err.stack);
                            console.error('❌ [ERROR] Registration attempted with eventObjId:', eventObjId, 'Type:', typeof eventObjId);
                            console.error('❌ [ERROR] Registration attempted with registrationId:', registrationId);
                            console.error('❌ [ERROR] Registration attempted with fields:', fields);
                            if (isAjaxRequest) {
                                return res.status(500).json({success: false, message: 'Error saving registration', error: err.message, stack: err.stack});
                            } else {
                                return res.status(500).send('Error saving registration: ' + err.message);
                            }
                        }
                        if (!err && savedData) {
                            // Confirm event field type in DB
                            EventData.findById(savedData._id, function(findErr, foundDoc) {
                                if (findErr) {
                                    console.error('❌ Error finding saved registration in DB:', findErr);
                                } else if (!foundDoc) {
                                    console.error('❌ Registration not found in DB after save:', savedData._id);
                                } else {
                                    console.log('✅ Registration confirmed in DB:', foundDoc);
                                    console.log('✅ [DEBUG] Saved registration event field type:', typeof foundDoc.event, 'Value:', foundDoc.event);
                                }
                            });
                        }
                        // ...existing code...
                ...
                console.error('❌ [FATAL] Registration outer error:', outerErr);
                if (isAjaxRequest) {
                    return res.status(500).json({success: false, message: 'Fatal error saving registration', error: outerErr.message, stack: outerErr.stack});
                } else {
                    return res.status(500).send('Fatal error saving registration: ' + outerErr.message);
                }
            }
                    uploadImage: uploadImageFile,
                    uploadLogo: uploadLogoFile,
                ...
                eventData.save(function(err, savedData) {
                    ...
                    // Extra debug: Confirm document is in DB
                    var EventData = require('../models/event-data');
                    EventData.findById(savedData._id, function(findErr, foundDoc) {
                        if (findErr) {
                            console.error('❌ Error finding saved registration in DB:', findErr);
        ...
        if (fs.existsSync(confirmationHtmlPath)) {
            try {
                ...
                if(registrationId) {
                    ...
                    var mongoose = require('mongoose');
                    EventData.findOne({event: mongoose.Types.ObjectId(eventId), registrationId: registrationId}).exec(function(err, registration) {
