                        ...
                        if (savedData.email && savedData.email.trim() !== '') {
                            ...
                            if (transporter) {
                                ...
                                var emailHTML = (emailTemplate.htmlTemplate || generateDefaultEmailHTML())
                                    ...
                                    .replace(/\{\{logoUrl\}\}/g, (designData.header && designData.header.logo) ? `${req.protocol}://${req.get('host')}/event-designs/${eventId}/${designData.header.logo}` : '')
                                    .replace(/\{\{customBackground\}\}/g, designData.background ? `${req.protocol}://${req.get('host')}/event-designs/${eventId}/${designData.background}` : '');
...
router.get('/public-registration/:id/confirmation', function(req, res) {
    ...
    Event.findById(eventId).exec(function(err, event) {
        ...
        let confirmationLogoUrl = (designData.header && designData.header.logo) ? `${req.protocol}://${req.get('host')}/event-designs/${eventId}/${designData.header.logo}` : '';
        let confirmationCustomBackground = designData.background ? `${req.protocol}://${req.get('host')}/event-designs/${eventId}/${designData.background}` : '';
        if(registrationId) {
            var mongoose = require('mongoose');
            let eventQuery = [];
            if (mongoose.Types.ObjectId.isValid(eventId)) {
            ...
            EventData.findOne({$or: eventQuery}).exec(function(err, registration) {
                if(err) {
                    return res.render('event/registration-confirmation', {
                        ...
                        confirmationLogoUrl: logoUrl,
                        confirmationCustomBackground: customBackground
                    });
                }
                if(!registration) {
                    return res.render('event/registration-confirmation', {
                        ...
                        savedConfirmationHtml: savedConfirmationHtml,
                        confirmationLogoUrl: ogoUrl,
                        confirmationCustomBackground: confirmationCustomBackground
                    });
                }
                res.render('event/registration-confirmation', {
                    ...
                    confirmationLogoUrl: confirmationLogoUrl,
                    confirmationCustomBackground: confirmationCustomBackground
                });
            });
        } else {
