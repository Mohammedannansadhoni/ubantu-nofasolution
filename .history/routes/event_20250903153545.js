...
router.get('/public-registration/:id/confirmation', function(req, res) {
    ...
    Event.findById(eventId).exec(function(err, event) {
        ...
        // Build absolute URLs for logo/background
        var logoUrl = designData.header && designData.header.logo ? `${req.protocol}://${req.get('host')}/event-designs/${eventId}/${designData.header.logo}` : '';
        var customBackground = designData.background ? `${req.protocol}://${req.get('host')}/event-designs/${eventId}/${designData.background}` : '';
