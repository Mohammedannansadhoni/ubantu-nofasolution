...
router.get('/online-registration/:id/design-registration', function(req, res) {
    ...
    Event.findById(eventId).exec(function(err, event) {
        ...
        res.render('event/registration_page_designer', { 
            event: event,
            eventId: eventId,
...
// Route to save page design
// Route to save canvas and move to next step in workflow
router.post('/online-registration/:id/save-canvas', function(req, res) {
    var Event = require('../models/event');
    var eventId = req.params.id;
    var fs = require('fs');
    var path = require('path');
    
    console.log('🎯 Starting Save & Next workflow for event:', eventId);
    
    // Save the canvas data to the event
    Event.findByIdAndUpdate(eventId, {
        canvasHtml: req.body.canvasHtml,
        fieldsOrder: req.body.fieldsOrder,
        ...
        if(err) {
            ...
            return res.status(500).json({success: false, message: 'Error saving registration page'});
        }
        
        // Create event folder structure for assets
        var eventFolder = path.join(__dirname, '../public/registration/', eventId);
        var assetsFolder = path.join(eventFolder, 'assets');
        ...
        // Save the registration page HTML
        var registrationHtml = generateStandaloneRegistrationPage(req.body, event);
        fs.writeFileSync(path.join(eventFolder, 'registration.html'), registrationHtml);
        
        console.log('✅ Registration page saved, redirecting to confirmation designer');
        
        res.json({
            success: true, 
            message: 'Registration page saved successfully!',
            redirectUrl: `/event/online-registration/${eventId}/design-confirmation`
        });
    });
});

function generateStandaloneRegistrationPage(canvasData, event) {
    // Generate a complete standalone HTML page from the canvas data
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>${canvasData.eventTitle || event.eventName} - Registration</title>
    <style>
        body { margin: 0; font-family: Arial, sans-serif; background: #f5f5f5; }
        .registration-container { display: flex; justify-content: center; align-items: center; min-height: 100vh; padding: 20px; }
        .registration-card { background: white; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.1); padding: 40px; max-width: 500px; width: 100%; }
        .event-logo { width: 110px; height: 60px; margin: 0 auto 20px; display: block; }
        .event-title { font-size: 24px; font-weight: bold; text-align: center; margin-bottom: 10px; color: #333; }
        .event-header { font-size: 16px; text-align: center; margin-bottom: 30px; color: #666; }
        .registration-form { }
        ...
        .required { color: red; }
        .submit-btn { width: 100%; padding: 15px; background: #007bff; color: white; border: none; border-radius: 6px; font-size: 16px; font-weight: bold; cursor: pointer; }
        .submit-btn:hover { background: #0056b3; }
        .event-footer { text-align: center; margin-top: 20px; color: #666; font-size: 12px; }
    </style>
</head>
<body style="${canvasData.backgroundImage ? `background-image: ${canvasData.backgroundImage}; background-size: cover; background-position: center;` : ''}">
...
router.post('/online-registration/:id/save-design', function(req, res) {
    var eventId = req.params.id;
    var pageType = req.body.pageType || 'registration'; // 'registration' or 'confirmation'
    var pageContent = req.body.pageContent;
    var nextStep = req.body.nextStep; // true if this is a Save & Next action
    ...
    if (!fs.existsSync(path.join(eventFolder, 'assets'))) {
        fs.mkdirSync(path.join(eventFolder, 'assets'), { recursive: true });
    }
    
    var fileName = pageType === 'confirmation' ? 'confirmation.html' : 'registration.html';
    var filePath = path.join(eventFolder, fileName);
    
    // Generate complete HTML page
    var completeHtml = generateCompletePageHtml(pageContent, pageType, eventId);
    fs.writeFileSync(filePath, completeHtml);
    
    console.log(`📄 ${pageType} page saved to: ${filePath}`);
    
    res.json({
        success: true, 
        message: `${pageType} page saved successfully`,
        nextStep: nextStep
    });
});

function generateCompletePageHtml(content, pageType, eventId) {
    var title = pageType === 'confirmation' ? 'Registration Confirmation' : 'Event Registration';
...
<body>
    ...
    <script>
        // Handle form submission for registration pages
        if ('${pageType}' === 'registration') {
            $('.registration-form').on('submit', function(e) {
                ...
                $.ajax({
                    url: '/event/${eventId}/public-register',
                    method: 'POST',
                    data: formData,
                    success: function(response) {
                        // Redirect to confirmation page
                        window.location.href = '/registration/${eventId}/confirmation.html';
                    },
                    error: function() {
...
router.post('/online-registration/:id/generate-link', function(req, res) {
    ...
    var eventFolder = path.join(__dirname, '../public/registration/', eventId);
    var registrationFile = path.join(eventFolder, 'registration.html');
    var confirmationFile = path.join(eventFolder, 'confirmation.html');
    
    if (!fs.existsSync(registrationFile)) {
        return res.status(400).json({
            success: false, 
            message: 'Registration page not found. Please save the registration page first.'
        });
    }
    ...
    if (!fs.existsSync(confirmationFile)) {
        return res.status(400).json({
            success: false, 
            message: 'Confirmation page not found. Please save the confirmation page first.'
        });
    }
    ...
    Event.findByIdAndUpdate(eventId, {
        ...
        // Generate the public URL that points to the saved HTML files
        var publicUrl = `/registration/${eventId}/registration.html`;
        
        console.log(`🌐 Public registration link generated for ${event.eventName}`);
        console.log(`📁 Files saved in: ${eventFolder}`);
...
// NEW ASSET MANAGEMENT ROUTES FOR EVENT FOLDERS

// Route to save design assets to event folder
router.post('/:id/save-design', function(req, res) {
    var eventId = req.params.id;
    var assetsData = req.body;
    var fs = require('fs');
    var path = require('path');
    
    console.log('🎨 Saving design assets for event:', eventId);
    ...
    try {
        ...
        var configFile = path.join(assetsFolder, 'design-config.json');
        var configData = {
            eventTitle: assetsData.eventTitle,
            eventHeader: assetsData.eventHeader,
            eventFooter: assetsData.eventFooter,
            formNote: assetsData.formNote,
            fieldsOrder: assetsData.fieldsOrder,
            canvasStyles: assetsData.canvasStyles,
            headerContent: assetsData.headerContent,
            footerContent: assetsData.footerContent,
            savedAt: new Date()
        };
        fs.writeFileSync(configFile, JSON.stringify(configData, null, 2));
        
        // Save logo if present
        if (assetsData.logoSrc && assetsData.logoSrc.startsWith('data:')) {
            var logoBuffer = Buffer.from(assetsData.logoSrc.split(',')[1], 'base64');
            var logoPath = path.join(imagesFolder, 'logo.png');
            fs.writeFileSync(logoPath, logoBuffer);
            configData.logoUrl = `/registration/${eventId}/assets/images/logo.png`;
        }
        
        // Save background image if present
        if (assetsData.backgroundImage && assetsData.backgroundImage.includes('data:')) {
            var bgImageData = assetsData.backgroundImage.match(/url\(['"]?(data:[^'"]+)['"]?\)/);
            if (bgImageData && bgImageData[1]) {
                var bgBuffer = Buffer.from(bgImageData[1].split(',')[1], 'base64');
                var bgPath = path.join(imagesFolder, 'background.jpg');
                fs.writeFileSync(bgPath, bgBuffer);
                configData.backgroundImageUrl = `/registration/${eventId}/assets/images/background.jpg`;
            }
        }
        ...
        console.log('✅ Design assets saved to:', eventFolder);
        
        // Determine next step URL
        var redirectUrl = `/event/online-registration/${eventId}/confirmation`;
        
        res.json({
            success: true,
            message: 'Design saved successfully to event folder',
        ...
        res.status(500).json({
            success: false,
            message: 'Failed to save design assets: ' + error.message
        });
    }
});

// Route to save general assets
router.post('/:id/save-assets', function(req, res) {
    var eventId = req.params.id;
    var assetsData = req.body;
    var fs = require('fs');
    var path = require('path');
    
    console.log('💾 Saving assets for event:', eventId);
    
    try {
        // Create event folder structure - same logic as save-design
        var eventFolder = path.join(__dirname, '../public/registration/', eventId);
        var assetsFolder = path.join(eventFolder, 'assets');
        var imagesFolder = path.join(assetsFolder, 'images');
        ...
        console.log('✅ Assets saved to:', eventFolder);
        ...
        res.status(500).json({
            success: false,
            message: 'Failed to save assets: ' + error.message
        });
    }
});

// Route to create assets and generate public URL
router.post('/:id/create-assets', function(req, res) {
    var eventId = req.params.id;
    var assetsData = req.body;
    var fs = require('fs');
    var path = require('path');
    ...
    console.log('🌐 Creating assets and generating public URL for event:', eventId);
    ...
    try {
        ...
        // Generate a basic public registration HTML file
        var publicHtmlContent = generatePublicRegistrationHtml(eventId, configData);
        var publicHtmlPath = path.join(eventFolder, 'index.html');
        fs.writeFileSync(publicHtmlPath, publicHtmlContent);
        
        // Update event with public registration enabled
        Event.findByIdAndUpdate(eventId, {
            publicRegistrationEnabled: true,
            registrationLinkGenerated: true,
            registrationLinkCreatedAt: new Date()
            ...
            if (err) {
                ...
                console.log('✅ Event updated with public registration enabled');
            }
        });
        
        console.log('✅ Assets created and public URL generated:', eventFolder);
        
        var publicUrl = `/registration/${eventId}/`;
        
        res.json({
            success: true,
            message: 'Event assets created and public URL generated',
            publicUrl: publicUrl,
            eventFolder: eventFolder
        });
        ...
        res.status(500).json({
            success: false,
            message: 'Failed to create assets: ' + error.message
        });
    }
});

// Route to load saved assets
router.get('/:id/load-assets', function(req, res) {
    var eventId = req.params.id;
    var fs = require('fs');
    var path = require('path');
    
    try {
        var configFile = path.join(__dirname, '../public/registration/', eventId, 'assets', 'design-config.json');
        
        if (fs.existsSync(configFile)) {
            var configData = JSON.parse(fs.readFileSync(configFile, 'utf8'));
            console.log('✅ Loaded saved assets for event:', eventId);
            res.json(configData);
        } else {
            res.status(404).json({
                success: false,
                message: 'No saved assets found'
            });
        }
    } catch (error) {
...
// Function to generate public registration HTML
function generatePublicRegistrationHtml(eventId, configData) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>${configData.eventTitle || 'Event Registration'}</title>
    <style>
        body {
            margin: 0;
            font-family: Arial, sans-serif;
            background: #f5f5f5;
            ${configData.backgroundImageUrl ? `background-image: url('${configData.backgroundImageUrl}'); background-size: cover; background-position: center;` : ''}
        }
...
<body>
    <div class="registration-container">
        <div class="registration-card">
            ${configData.logoUrl ? `<img src="${configData.logoUrl}" class="event-logo" alt="Event Logo">` : ''}
            <div class="event-title">${configData.eventTitle || 'Event Registration'}</div>
            <div class="event-header">${configData.eventHeader || 'Welcome to Our Event Registration'}</div>
            
            <form class="registration-form" action="/event/${eventId}/public-register" method="POST">
                <!-- Registration fields will be populated by backend -->
                <div class="form-note">${configData.formNote || 'Please provide accurate information for registration.'}</div>
                <input type="submit" value="REGISTER">
            </form>
            
            <div class="event-footer">${configData.eventFooter || 'Thank you for registering!'}</div>
        </div>
    </div>
</body>
</html>`;
...
router.get('/register/:id', function(req, res) {
    ...
    Event.findById(eventId).exec(function(err, event) {
        if(err || !event || !event.onlineRegistrationEnabled) {
            return res.status(404).send('Registration not found or not available');
        }
        
        // Serve the custom registration page
        var registrationFile = path.join(__dirname, '../public/registration/', eventId, 'registration.html');
        
        if(fs.existsSync(registrationFile)) {
            var content = fs.readFileSync(registrationFile, 'utf8');
            res.send(content);
        } else {
            // Fallback to default registration page
            res.render('event/default_registration', {event: event, eventId: eventId});
        }
    });
});
    ...
    Event.findById(eventId).exec(function(err, event) {
        ...
        BadgeCategory.find({event: eventId, isActive: true}).exec(function(err, badgeCategories) {
            ...
            Country.find({isActive: true}).exec(function(err, countries) {
                ...
                // Check if there's saved canvas configuration
                if(event.canvasHtml) {
    ...
    Event.findById(eventId).exec(function(err, event) {
        ...
        Sequence.findOneAndUpdate(
            ...
            eventData.save(function(err, savedData) {
                ...
                // You can add email sending functionality using nodemailer or similar
