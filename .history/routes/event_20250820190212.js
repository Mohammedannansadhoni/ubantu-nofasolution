...
router.post('/online-registration/:id/save-design', function(req, res) {
    ...
    Event.findById(eventId).exec(function(err, event) {
        ...
        try {
            ...
            var designData = {
                header: {
                    exists: req.body.headerExists === 'true',
                    logoAlignment: req.body.logoAlignment || 'center',
                    logo: null
                },
                footer: {
                    exists: req.body.footerExists === 'true',
...
router.get('/public-registration/:id', function(req, res) {
    ...
    Event.findById(eventId).exec(function(err, event) {
        ...
        // Check for saved design first
        var eventFolder = path.join(__dirname, '../public/event-designs/', eventId);
        var registrationHtmlPath = path.join(eventFolder, 'registration.html');
        var designJsonPath = path.join(eventFolder, 'design.json');
        
        // If saved registration HTML exists, serve it directly
        ...
        // If design.json exists but no HTML, regenerate the HTML
        if (fs.existsSync(designJsonPath)) {
            try {
                var designData = JSON.parse(fs.readFileSync(designJsonPath, 'utf8'));
                console.log('📋 Found design.json, regenerating HTML for event:', eventId);
                
                var completeRegistrationHtml = generateCompleteRegistrationHTML(designData, event, eventId);
                
                // Save the generated HTML for next time
                fs.writeFileSync(registrationHtmlPath, completeRegistrationHtml);
                
                res.setHeader('Content-Type', 'text/html');
                return res.send(completeRegistrationHtml);
                
            } catch (error) {
                console.warn('❌ Error processing design.json:', error.message);
                // Fall through to template rendering
            }
        }
