...
router.post('/event-fields', function (req, res) {
    ...
    Event.findById(eventId, function (err, event) {
        ...
        fieldNames.forEach(function(fieldName) {
            ...
            var checked = req.body[fieldKey] ? true : false;
            event[fieldKey] = checked;
            console.log('[FIELD DEBUG] Saving', fieldKey, '=', checked);
        });

        event.save(function (err, result) {
            if (err) {
                console.error('[FIELD DEBUG] Error saving event fields:', err);
            } else {
                console.log('[FIELD DEBUG] Event fields saved successfully:', result._id);
            }
...
function generateCompleteRegistrationHTML(designData, event, eventId) {
    ...
    visibleFields.forEach(function(fieldDef, index) {
        ...
        var fieldHtml = '';
        // Fix for image/logo fields
        if (fieldDef.name === 'uploadImage' || fieldDef.name === 'uploadLogo') {
            fieldHtml = '<input type="file" name="' + fieldDef.name + '" ' + requiredAttr + '>';
        } else {
                ...
                BadgeCategory.find({isActive: true}).exec(function(err, badgeCategories) {
                    ...
                    Country.find({isActive: true}).exec(function(err, countries) {
                        ...
                        fieldDefinitions.forEach(function(fieldDef) {
                            var showInOnlineRegistration = event[fieldDef.name + '_showInOnlineRegistration'];
                            var isMandatory = event[fieldDef.name + '_isMandatory'];
                            // For uploadImage and uploadLogo, check flag and always include if set
                            if((fieldDef.name === 'uploadImage' && event['uploadImage_showInOnlineRegistration']) ||
                               (fieldDef.name === 'uploadLogo' && event['uploadLogo_showInOnlineRegistration'])) {
                                fields.push({
                            ...
                            } else if(showInOnlineRegistration && fieldDef.name !== 'uploadImage' && fieldDef.name !== 'uploadLogo') {
                                fields.push({
                                    fieldName: fieldDef.name,
                                    fieldLabel: fieldDef.label,
                                    fieldType: fieldDef.type,
                    ...
                    if (savedData.email && savedData.email.trim() !== '') {
                        ...
                        if (transporter) {
                            ...
                            var emailHTML = (emailTemplate.htmlTemplate || generateDefaultEmailHTML())
                                // Support both curly and single brace variables from editor
                                .replace(/\{\{fullName\}\}|\{userName\}/g, savedData.fullName || (savedData.firstName + ' ' + savedData.lastName) || 'Participant')
                                .replace(/\{\{firstName\}\}/g, savedData.firstName || '')
                                .replace(/\{\{lastName\}\}/g, savedData.lastName || '')
                                .replace(/\{\{email\}\}/g, savedData.email || '')
                                .replace(/\{\{eventName\}\}|\{eventName\}/g, event.eventName || '')
                                .replace(/\{\{registrationId\}\}|\{registrationId\}/g, registrationId || '')
                                .replace(/\{\{companyName\}\}/g, savedData.companyName || '')
                                .replace(/\{\{badgeCategory\}\}/g, savedData.badgeCategory || '')
                                .replace(/\{\{registrationDate\}\}|\{registrationDate\}/g, moment(savedData.regDate).format('MMMM DD, YYYY'))
                                .replace(/\{\{confirmationUrl\}\}/g, confirmationUrl)
                                .replace(/\{\{qrCode\}\}|\{qrCode\}/g, savedData.qrCode || '')
                                .replace(/\{\{qrCodeShort\}\}|\{qrCodeShort\}/g, savedData.qrCode ? savedData.qrCode.toString().slice(-6) : '')
                                .replace(/\{\{barcode\}\}|\{barcode\}/g, savedData.barcode || '')
