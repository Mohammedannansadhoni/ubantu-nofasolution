...
function generateCompleteRegistrationHTML(canvasHtml, event, eventId) {
    ...
    // Create complete registration page by updating the saved canvas HTML with a proper form
    var completeHtml = canvasHtml;
    
    // Add a placeholder div where the form will be inserted
    if (!completeHtml.includes('id="registration-form-placeholder"')) {
        // Add form placeholder at the end of canvas content
        completeHtml = completeHtml.replace(
            /<\/div>\s*<\/div>\s*$/,
            '<div id="registration-form-placeholder"></div></div></div>'
        );
    }
...
'            z-index: 1000;' +
'        }' +
'        .registration-form-container .event-title {' +
'            text-align: center;' +
'            margin-bottom: 15px;' +
'            font-size: 18px;' +
'            font-weight: bold;' +
    ...
    Event.findById(eventId).exec(function(err, event) {
        ...
        BadgeCategory.find({event: eventId, isActive: true}).exec(function(err, badgeCategories) {
            ...
            Country.find({isActive: true}).exec(function(err, countries) {
                ...
                var eventFolder = path.join(__dirname, '../public/event-designs/', eventId);
                var registrationHtmlPath = path.join(eventFolder, 'registration.html');
                var savedDesignHtml = null;
                
                if (fs.existsSync(registrationHtmlPath)) {
                    try {
                        savedDesignHtml = fs.readFileSync(registrationHtmlPath, 'utf8');
                        console.log('✅ Found saved registration design for event:', eventId);
                    } catch (error) {
                        console.warn('❌ Error reading saved design, falling back to default:', error.message);
                    }
                }
                
                console.log('📋 Using template with', savedDesignHtml ? 'custom design' : 'default design');
                ...
                var templateData = {
                    ...
                    savedDesignHtml: savedDesignHtml // Add the saved design HTML
                };
