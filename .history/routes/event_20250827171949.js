...
router.post('/online-registration/:id/save-design', function(req, res) {
    ...
    form.parse(req, function (err, fields, files) {
        ...
        var pageType = fields.pageType || 'registration';
        console.log('Saving design for event:', eventId, 'Page type:', pageType);
        console.log('Request fields keys:', Object.keys(fields));

        Event.findById(eventId).exec(function(err, event) {
            // Debug: log incoming logoUrl and backgroundImage
            var debugLogoUrl = fields.logoUrl;
            var debugBackgroundImage = fields.backgroundImage;
            console.log('DEBUG: Received logoUrl:', debugLogoUrl);
            console.log('DEBUG: Received backgroundImage:', debugBackgroundImage);
        if(err || !event) {
        ...
        try {
            ...
            var logoUrl = typeof fields.logoUrl === 'string' ? fields.logoUrl : '';
            var designData = {
                header: {
                    exists: (fields.headerExists === 'true' || fields.headerExists === true) || false,
                    logoAlignment: fields.logoAlignment || 'center',
                    logo: null
                },
                footer: {
                    exists: (fields.footerExists === 'true' || fields.footerExists === true) || false,
                    content: fields.footerContent || ''
                },
                background: null,
                content: {
                    eventTitle: fields.eventTitle || event.eventName || 'Sample Event',
                    eventHeader: fields.eventHeader || 'Welcome to Our Event Registration',
                    eventFooter: fields.eventFooter || 'Thank you for registering!'
                },
            ...
            if (logoUrl && logoUrl.startsWith('data:')) {
                var savedLogoPath = saveBase64Image(logoUrl, 'header-logo');
                console.log('DEBUG: Saved header logo to:', savedLogoPath);
                designData.header.logo = savedLogoPath;
            } else if (logoUrl) {
                console.log('DEBUG: Using header logo as:', logoUrl);
                designData.header.logo = logoUrl;
            } else {
                designData.header.logo = '';
            }
            
            // Save background image if provided
            var backgroundImage = typeof fields.backgroundImage === 'string' ? fields.backgroundImage : '';
            if (backgroundImage && backgroundImage.startsWith('data:')) {
                var savedBgPath = saveBase64Image(backgroundImage, 'background');
                console.log('DEBUG: Saved background image to:', savedBgPath);
                designData.background = savedBgPath;
            } else if (backgroundImage) {
                console.log('DEBUG: Using background image as:', backgroundImage);
                designData.background = backgroundImage;
            } else {
            ...
            var canvasHtml = fields.canvasHtml || '';
            var updateData = {
                ...
                if(registrationId) {
                    ...
                    EventData.findOne({$or: eventQuery}).exec(function(err, registration) {
                        ...
                        res.render('event/registration-confirmation', {
                            ...
                            confirmationLogoUrl: event.confirmationLogoUrl || event.customLogoUrl,
                            confirmationCustomBackground: event.confirmationCustomBackground || event.customBackground,
                            confirmationBarcode: registration.barcode || '',
                            confirmationQrCode: registration.qrCode || '',
                            confirmationQrCodeVCard: registration.qrCodeVCard || ''
                        });
                    });
                } else {
