...
router.post('/online-registration/:id/save-design', function(req, res) {
    ...
    if (pageType === 'confirmation') {
        ...
        fs.writeFileSync(registrationHtmlPath, canvasHtml);
...
router.post('/online-registration/:id/generate-link', function(req, res) {
    ...
    Event.findByIdAndUpdate(eventId, {
        ...
        var publicUrl = `${req.protocol}://${req.get('host')}/event/register/${eventId}`;
...
router.get('/register/:id', function(req, res) {
    ...
    Event.findById(eventId).exec(function(err, event) {
        ...
        console.log('Loading public registration for event:', eventId);
        
        // Check for saved registration design in new event-designs folder
        var registrationFile = path.join(__dirname, '../public/event-designs/', eventId, 'registration.html');
        
        if(fs.existsSync(registrationFile)) {
            console.log('✅ Found saved registration HTML file:', registrationFile);
            var content = fs.readFileSync(registrationFile, 'utf8');
            
            // Update any relative URLs to work with the new structure
            content = content.replace(/\/event-designs\//g, '/event/event-designs/');
            
            res.send(content);
        } else {
            console.log('ℹ️ No saved registration file, redirecting to public-registration route');
            // Redirect to the public-registration route which has full functionality
