...
router.get('/badge-layout', function(req,res){
    ...
    Event.findById(eventId, function (err, event) {
        ...
        Font.find({isActive: true, fontName: {$exists: true, $ne: null, $ne: ''}}).sort({created: -1}).exec(function(err, customFonts) {
            if(err) {
                console.error('Error loading custom fonts for badge layout:', err);
                customFonts = [];
            } else {
                console.log('Custom fonts loaded for badge layout:', customFonts.length, customFonts.map(f => f.fontName));
            }
...
router.get('/manage-fonts', function(req, res) {
    ...
    Font.find({isActive: true, fontName: {$exists: true, $ne: null, $ne: ''}}).sort({created: -1}).exec(function(err, fonts) {
        if(err) {
            console.error('Error loading fonts:', err);
            messages.push('Error loading fonts');
            fonts = [];
        } else {
            console.log('Fonts found:', fonts.length, fonts.map(f => f.fontName));
        }
...
// Original font upload route with formidable
router.post('/manage-fonts', function(req, res) {
    var messages = [];
    var Font = require('../models/font');
    var form = new formidable.IncomingForm();
    
    console.log('=== FONT UPLOAD POST REQUEST RECEIVED ===');
    ...
    form.parse(req, function (err, fields, files) {
        ...
        if(err) {
            ...
            messages.push('Error processing form data: ' + err.message);
            return res.redirect('/event/manage-fonts');
        }
        
        console.log('=== FORM PARSED SUCCESSFULLY ===');
        console.log('Raw fields:', fields);
        console.log('Raw files:', Object.keys(files));
        console.log('Files details:', Object.keys(files).map(key => ({
            key: key,
            name: files[key].name,
            size: files[key].size,
            type: files[key].type
        ...
        var fontType = fields.fontType;
        
        console.log('=== EXTRACTED FORM DATA ===');
        console.log('Font Name:', fontName);
        console.log('Font Type:', fontType);
        console.log('Font API:', fontApi);
        
        // Validation
        if(!fontName || fontName.trim() === '') {
            console.log('❌ Validation failed: Font name is required');
            messages.push('Font name is required');
            return Font.find({isActive: true, fontName: {$exists: true, $ne: null, $ne: ''}}).sort({created: -1}).exec(function(err, fonts) {
                res.render('event/manage-fonts', { 
                    messages: messages, 
                    hasErrors: messages.length > 0,
                    fonts: fonts || [] 
        ...
        if(!fontType || fontType.trim() === '') {
            console.log('❌ Validation failed: Font type is required');
            messages.push('Font type is required');
            return Font.find({isActive: true, fontName: {$exists: true, $ne: null, $ne: ''}}).sort({created: -1}).exec(function(err, fonts) {
                res.render('event/manage-fonts', { 
                    messages: messages, 
                    hasErrors: messages.length > 0,
        ...
        Font.findOne({fontName: fontName, isActive: true}, function(err, existingFont) {
            if(err) {
                console.error('Database error while checking existing font:', err);
                messages.push('Database error: ' + err.message);
                return Font.find({isActive: true, fontName: {$exists: true, $ne: null, $ne: ''}}).sort({created: -1}).exec(function(err, fonts) {
                    res.render('event/manage-fonts', { 
                        messages: messages, 
                        hasErrors: messages.length > 0,
            ...
            if(existingFont) {
                ...
                messages.push('Font with this name already exists');
                return Font.find({isActive: true, fontName: {$exists: true, $ne: null, $ne: ''}}).sort({created: -1}).exec(function(err, fonts) {
                    res.render('event/manage-fonts', { 
                        messages: messages, 
                        hasErrors: messages.length > 0,
            ...
            console.log('✅ Font name is unique, proceeding with creation');
            console.log('=== CREATING NEW FONT ===');
            var newFont = new Font();
            newFont.fontName = fontName.trim();
            newFont.fontFamily = fontName.trim(); // Use font name as family name
            newFont.fontType = fontType;
            newFont.isActive = true;
            
            console.log('New font object created:', {
                fontName: newFont.fontName,
                fontFamily: newFont.fontFamily,
                fontType: newFont.fontType,
                isActive: newFont.isActive
            });
            
            if(fontType === 'google' && fontApi && fontApi.trim() !== '') {
                console.log('=== PROCESSING GOOGLE FONT API ===');
                newFont.googleFontApi = fontApi.trim();
                
                console.log('Saving Google Font to database...');
                newFont.save(function(err, result) {
                    console.log('=== GOOGLE FONT SAVE RESULT ===');
                    if(err) {
                        console.error('❌ Save error:', err);
                        messages.push('Error saving Google Font to database: ' + err.message);
                    } else {
                        console.log('✅ Google Font saved successfully:', result._id);
                        messages.push('Google Font "' + fontName + '" added successfully');
                    }
                    res.redirect('/event/manage-fonts');
                });
            } else if(fontType === 'google-file' && files.googleFontFile && files.googleFontFile.name) {
                console.log('=== PROCESSING GOOGLE FONT FILE UPLOAD ===');
                console.log('File details:', {
                    name: files.googleFontFile.name,
                    size: files.googleFontFile.size,
                    type: files.googleFontFile.type,
                    path: files.googleFontFile.path
                });
                
                // Handle Google font file upload
                var oldpath = files.googleFontFile.path;
                var fileName = Date.now() + '_' + files.googleFontFile.name.replace(/\s+/g, '_');
                ...
                fs.readFile(oldpath, function (err, data) {
                    if (err) {
                        ...
                        messages.push('Error reading font file: ' + err.message);
                        return res.redirect('/event/manage-fonts');
                    }
                    ...
                    fs.writeFile(newpath, data, function (err) {
                        if (err) {
                            ...
                            messages.push('Error saving font file: ' + err.message);
                            return res.redirect('/event/manage-fonts');
                        }
                        
                        console.log('✅ File saved successfully to:', newpath);
                        newFont.fontFile = fileName;
                        
                        console.log('Saving Google Font file to database...');
                        newFont.save(function(err, result) {
                            console.log('=== GOOGLE FONT FILE SAVE RESULT ===');
                            if(err) {
                                console.error('❌ Save error:', err);
                                messages.push('Error saving Google Font file to database: ' + err.message);
                            } else {
                                console.log('✅ Google Font file saved successfully:', result._id);
                                messages.push('Google Font file "' + fontName + '" uploaded successfully');
                            }
                            res.redirect('/event/manage-fonts');
                        });
                        ...
                        fs.unlink(oldpath, function (err) {
                            if (err) console.log('Warning: Error deleting temp file:', err);
                        });
                    });
                });
            } else if(fontType === 'custom' && files.customFontFile && files.customFontFile.name) {
                console.log('=== PROCESSING CUSTOM FONT FILE UPLOAD ===');
                console.log('File details:', {
                    name: files.customFontFile.name,
                    size: files.customFontFile.size,
                    type: files.customFontFile.type,
                    path: files.customFontFile.path
                });
                
                // Handle custom font file upload
                var oldpath = files.customFontFile.path;
                var fileName = Date.now() + '_' + files.customFontFile.name.replace(/\s+/g, '_');
                var newpath = path.join(__dirname, '../public/uploads/fonts/') + fileName;
                ...
                fs.readFile(oldpath, function (err, data) {
                    ...
                    fs.writeFile(newpath, data, function (err) {
                        ...
                        console.log('Saving custom font to database...');
                        newFont.save(function(err, result) {
                            console.log('=== CUSTOM FONT SAVE RESULT ===');
                            if(err) {
                                console.error('❌ Save error:', err);
                                messages.push('Error saving custom font to database: ' + err.message);
                            } else {
                                console.log('✅ Custom font saved successfully:', result._id);
                                messages.push('Custom font "' + fontName + '" uploaded successfully');
                            }
                            res.redirect('/event/manage-fonts');
                        });
                ...
                console.log('Validation details:', {
                    fontType: fontType,
                    hasGoogleApi: !!(fontApi && fontApi.trim()),
                    hasGoogleFile: !!(files.googleFontFile && files.googleFontFile.name),
                    hasCustomFile: !!(files.customFontFile && files.customFontFile.name)
                });
                
                var errorMsg = 'Invalid form data. ';
                if(fontType === 'google') {
                    errorMsg += 'Please provide Google Font API link.';
                } else if(fontType === 'google-file') {
                    errorMsg += 'Please select a Google Font file.';
                } else if(fontType === 'custom') {
                    errorMsg += 'Please select a custom font file.';
                } else {
                    errorMsg += 'Please select a valid font type and provide required data.';
                }
