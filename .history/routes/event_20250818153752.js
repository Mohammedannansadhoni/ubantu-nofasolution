...
// Route to display online registration page
router.get('/online-registration/:id', function(req, res) {
    ...
    Event.findById(eventId).exec(function(err, event) {
        ...
        console.log(`Loading online registration page for event: ${event.eventName}`);
        ...
        BadgeCategory.find({isActive: true}).exec(function(err, badgeCategories) {
            ...
            Country.find({isActive: true}).exec(function(err, countries) {
                ...
                var fieldDefinitions = [
                    ...
                    {name: 'title', label: event.title_label, type: 'dropdown', options: ['Mr.', 'Mrs.', 'Ms.', 'Dr.', 'Prof.']},
                    ...
                    {name: 'country', label: event.country_label, type: 'dropdown', options: countries.map(c => c.name)},
                    {name: 'poBox', label: event.poBox_label, type: 'text'},
                    {name: 'postalCode', label: event.postalCode_label, type: 'text'},
                    {name: 'badgeCategory', label: event.badgeCategory_label, type: 'dropdown', options: badgeCategories.map(bc => bc.categoryCode)},
                    {name: 'regType', label: event.regType_label, type: 'dropdown', options: ['Online', 'Onsite']},
                ...
                fieldDefinitions.forEach(function(fieldDef) {
                    ...
                    if(showInOnlineRegistration) {
                        fields.push({
                            ...
                            dropdownOptions: fieldDef.options || []
                        });
                    }
                });
                
                console.log(`Generated ${fields.length} fields for online registration`);
