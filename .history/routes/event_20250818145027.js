...
router.post('/manage-fonts', function(req, res) {
    ...
    form.parse(req, function (err, fields, files) {
        ...
        Font.findOne({fontName: fontName, isActive: true}, function(err, existingFont) {
            ...
            newFont.fontFamily = fontName.trim();
            newFont.fontType = fontType || 'google';
            newFont.isActive = true;
            
            if(fontType === 'google' && fontApi && fontApi.trim() !== '') {
                newFont.googleFontApi = fontApi.trim();
            } else {
                newFont.googleFontApi = 'https://fonts.googleapis.com/css2?family=Open+Sans';
            }
            
            newFont.save(function(err, result) {
                if(err) {
                    console.error('Save error:', err);
                    messages.push('Error saving font: ' + err.message);
                } else {
                    console.log('Font saved successfully:', result._id);
                    messages.push('Font "' + fontName + '" added successfully');
                }
                res.redirect('/event/manage-fonts');
            });
        });
