...
router.get('/download/:id', function(req,res){
    var messages = [];
        ...
        const specification = {
            ...
            qrCode: {displayName: 'QR Code', headerStyle: styles.headerDark, width: 120},
            uploadImage: {displayName: 'Upload Image', headerStyle: styles.headerDark, width: 120},
            uploadLogo: {displayName: 'Upload Logo', headerStyle: styles.headerDark, width: 120},
...
function generateCompleteRegistrationHTML(designData, event, eventId) {
    ...
    var fieldDefinitions = [
        ...
        {name: 'regType', label: event.regType_label || 'Registration Type', type: 'text'},
        {name: 'regDate', label: event.regDate_label || 'Registration Date', type: 'date'},
        {name: 'badgePrintDate', label: event.badgePrintDate_label || 'Badge Print Date', type: 'date'},
        {name: 'modifiedDate', label: event.modifiedDate_label || 'Modified Date', type: 'date'},
        {name: 'statusFlag', label: event.statusFlag_label || 'Status Flag', type: 'text'},
        {name: 'backoffice', label: event.backoffice_label || 'Backoffice', type: 'text'},
        {name: 'comment1', label: event.comment1_label || 'Comment 1', type: 'text'},
        {name: 'comment2', label: event.comment2_label || 'Comment 2', type: 'text'},
        {name: 'comment3', label: event.comment3_label || 'Comment 3', type: 'text'},
        {name: 'comment4', label: event.comment4_label || 'Comment 4', type: 'text'},
        {name: 'comment5', label: event.comment5_label || 'Comment 5', type: 'text'},
        {name: 'comment6', label: event.comment6_label || 'Comment 6', type: 'text'},
        {name: 'comment7', label: event.comment7_label || 'Comment 7', type: 'text'},
        {name: 'comment8', label: event.comment8_label || 'Comment 8', type: 'text'},
        {name: 'comment9', label: event.comment9_label || 'Comment 9', type: 'text'},
        {name: 'qrCode', label: event.qrCode_label || 'QR Code', type: 'text'},
        {name: 'registrationForm', label: event.registrationForm_label || 'Registration Form', type: 'form'},
                ...
                eventData.save(function(err, savedData) {
                    ...
                    if (savedData.email && savedData.email.trim() !== '') {
                        ...
                        if (transporter) {
                            ...
                            // If no custom email template, build HTML using design.json for logo/background
                            var logoUrl = '';
                            var backgroundImage = '';
                            var fs = require('fs');
                            var path = require('path');
                            var designPath = path.join(eventFolder, 'design.json');
                            if (fs.existsSync(designPath)) {
                                try {
                                    var designData = JSON.parse(fs.readFileSync(designPath, 'utf8'));
                                    logoUrl = designData.header && designData.header.logo ? designData.header.logo : '';
                                    backgroundImage = designData.background || '';
                                } catch (error) {
                                    console.warn('[EMAIL DEBUG] Error reading design.json:', error.message);
                                }
                            }
                            var emailHTML = (emailTemplate.htmlTemplate || generateDefaultEmailHTML())
                                ...
                                .replace(/\{qrCode\}/g, savedData.qrCode || '')
                                .replace(/\{\{qrCodeShort\}\}/g, savedData.qrCode ? savedData.qrCode.toString().slice(-6) : '')
                                .replace(/\{qrCodeShort\}/g, savedData.qrCode ? savedData.qrCode.toString().slice(-6) : '')
                            ...
                            console.log('[EMAIL DEBUG] Final email HTML being sent:');
            ...
            try {
                ...
                if(registrationId) {
                    ...
                    EventData.findOne({$or: eventQuery}).exec(function(err, registration) {
                        ...
                        // Always assign barcode and qrCode if missing
                        const assignCodesAndRender = (reg) => {
                            res.render('event/registration-confirmation', {
                                ...
                                savedConfirmationHtml: savedConfirmationHtml,
                                qrCode: reg.qrCode || '',
                                qrCodeShort: (reg.qrCode ? reg.qrCode.toString().slice(-6) : ''),
                        ...
                        if(!registration.barcode || !registration.qrCode) {
                            ...
                            if(!registration.barcode) {
                                promises.push(new Promise((resolve, reject) => {
                                    Sequence.findOneAndUpdate({name:'barcode'}, {$inc:{value:1}}, {new:true}, function(err, barcodeSeq){
                                        ...
                                        update.barcode = barcodeSeq ? barcodeSeq.value : '19299259221626';
                                        resolve();
                                    });
                                }));
                            }
                            if(!registration.qrCode) {
                                promises.push(new Promise((resolve, reject) => {
                                    Sequence.findOneAndUpdate({name:'qrCode'}, {$inc:{value:1}}, {new:true}, function(err, qrCodeSeq){
                                        if(err) return reject(err);
                                        update.qrCode = qrCodeSeq ? qrCodeSeq.value : '10000000000001';
                                        resolve();
                                    });
                                }));
                            }
                            Promise.all(promises).then(() => {
                                require('../models/event-data').findByIdAndUpdate(registration._id, update, {new:true}, function(err, updatedReg){
                                    if(err) {
                                        console.error('❌ Error updating registration with codes:', err);
                                        assignCodesAndRender(registration);
                                    } else {
                                        assignCodesAndRender(updatedReg);
                                    }
                                });
                            }).catch((err) => {
