    ...
    Event.findById(eventId, function (err, event) {
        ...
        Font.find({isActive: true}).sort({created: -1}).exec(function(err, customFonts) {
            ...
            res.render('event/badge-layout', {
                eventIdForPrint:eventIdForPrint,
                scripts:scripts,
                messages:messages,
        ...
        Font.find({isActive: true, fontName: {$exists: true, $ne: null, $ne: ''}}).sort({created: -1}).exec(function(err, customFonts) {
            ...
            res.render('event/print-badge-layout', {
                layout:'print-layout',
                messages:messages,
                hasErrors:messages.length>0,
                fields:fields,
...
function generateCompleteRegistrationHTML(designData, event, eventId) {
    ...
    var fields = [];
    // Always include all mandatory fields, and all fields marked for online registration
    fieldDefinitions.forEach(function(fieldDef) {
        ...
        if (showInOnlineRegistration || isMandatory) {
            fields.push({
                fieldName: fieldName,
...
// Fix for canvas HTML saving (add missing closing bracket and commas)
router.post('/online-registration/:id/generate-public-url', function(req, res) {
    var eventId = req.params.id;
    var configData = req.body;
    var Event = require('../models/event');
    Event.findByIdAndUpdate(eventId, {
        canvasHtml: configData.canvasHtml,
        customEventTitle: configData.eventTitle || null,
        customEventHeader: configData.eventHeader || null,
        customEventFooter: configData.eventFooter || null,
        customLogoUrl: configData.logoSrc || null,
        customBackground: configData.backgroundImage || null,
        fieldsOrder: configData.fieldsOrder || null
    }, {new: true}, function(err, event) {
        if(err) {
